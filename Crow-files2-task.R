...
#The example of GIS data (specifically North Carolina, NC)which includes the longitude and latitude on the map around the world by clicking
...
#Install & load  (geosphere), the  package for the geographic distance between two points with latitude/longitude coordinates
...
######################################################################################################
my_dist1 <- distHaversine(my_points, r=6378137)          # Calculate Haversine distance
my_dist1 # it is m therefore it should divide by 1000 to change to km 
Chch_Dunedin1=my_dist1/1000
Chch_Dunedin1
#######################################################################################
my_dist2 <- distVincentyEllipsoid(my_points, a=	6378137, b=6356752.3142, f=1/298.257223563)    # Calculate Vincenty ellipsoid distance
my_dist2   # it is m therefore it should divide by 1000 to change to km
Chch_Dunedin2=my_dist2/1000
Chch_Dunedin2
###########################################################
my_dist3 <- distGeo(c(172.54028,-43.54457 ), c(170.41992, -45.88818), a=6378137, f=1/298.257223563)    # Calculate the geodesic WGS84
